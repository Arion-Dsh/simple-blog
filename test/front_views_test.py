...
class FrontendViewsTest(BaseCase):
    ...
    def test_en_us_articles(self):
        ...
        self.assertIn('%s' % self.article_3.creat_time.strftime('%d-%m'), body)
        
    def test_article(self):
        url = '/zh-hans/%s/%s' %(self.article_1.creat_time.strftime('%d-%m'), self.article_1.id_no)
        response = self.fetch(url)
        body = response.body.decode()
    ...
    def test_chapter(self):
        url = '/novel/%s/%s' %(url_escape(self.novel.name), self.chapter_1.id_no)
        response = self.fetch(url)
        body = response.body.decode()
