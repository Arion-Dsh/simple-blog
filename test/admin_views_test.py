...
class AdminViewsTest (BaseCase):
    ...
    def test_home(self):
        ...
        response = self.fetch('/admin', method="POST", headers=self.headers, body=body)
    ...
    def test_article_about(self):
        ...
        body = urlencode(dict(
            ...
            active=1,
            category='zh-hans',
            creat_time='2015-06-14 16:07',
            md_content='test 1 2 3 <pre><code>def123</code></pre>',
        ))
        response = self.fetch('/admin/article/add', method="POST", headers=self.headers, body=body)
        ...
        self.assertIn('/zh-hans/%s/%s' %(article.creat_time.strftime('%d-%m'), article.id_no), 
                      response.body.decode())
        self.assertIn('/admin/article/%s/edit' %article.id_no, response.body.decode())
    ...
    def test_quote_about(self):
        ...
        response = self.fetch('/admin/quote/%s' % quote.id_no, headers=self.headers)
        self.assertIn(b'test quote 1', response.body)
        self.assertIn(b'arion_______', response.body)
        ...
        response = self.fetch('/admin/quote/%s' % quote.id_no, method="POST", headers=self.headers, body=body)
