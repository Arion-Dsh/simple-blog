...
func ArticlesGet(c jvmao.Context) error {

	cc := c.(*CustomContext)

	p := bindParams(cc)

	cid := cc.ParamValue("cid")
...
func ArticleGet(c jvmao.Context) error {
	cc := c.(*CustomContext)
	id, _ := primitive.ObjectIDFromHex(c.ParamValue("id"))
	...
	cursor := db.C(article).FindOne(
		ctx,
		bson.M{"_id": id, "category": cc.ParamValue("cid")},
		options.FindOne(),
	)
