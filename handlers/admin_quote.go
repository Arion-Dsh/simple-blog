...
func AdminQuotesGet(c jvmao.Context) error {
	cc := c.(*CustomContext)
...
//AdminQuotePost ...
func AdminQuotePost(c jvmao.Context) error {
	cc := c.(*CustomContext)
	q := new(models.Quote)

	qid := cc.ParamValue("qid")
	...
	return cc.Redirect(http.StatusSeeOther, cc.Request().URL.String())
}

//AdminQuoteEdit ...
func AdminQuoteEdit(c jvmao.Context) error {
	cc := c.(*CustomContext)
